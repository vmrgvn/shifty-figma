...
export function OverviewPage({ notification, schedules, wishes, onCreate, onOpenSchedule, onOpenWishes }: Props) {
  ...
  if (!active) return <TopLevelPageLayout width="default" title="Обзор" description="Главное о расписаниях и ситуациях, которые требуют внимания." notification={notification} onCreateSchedule={onCreate}><div className="cr-panel cr-empty"><div><div className="cr-block-illustration" aria-hidden="true">{Array.from({ length: 12 }, (_, index) => <i key={index} />)}</div><h2>Пока нет расписаний</h2><p>Создайте первое расписание — сотрудники, смены и пожелания будут собраны в одном месте.</p></div></div></TopLevelPageLayout>;
  ...
  return (
    <TopLevelPageLayout width="default" title="Обзор" description="Главное о расписаниях и ситуациях, которые требуют внимания." notification={notification} onCreateSchedule={onCreate}>
      <div className="cr-overview-columns">
        <div className="cr-overview-column cr-overview-column--primary">
          <section className="cr-panel cr-hero cr-overview-primary">
        ...
        <div className="cr-overview-column cr-overview-column--secondary">
          ...
          <section className="cr-panel cr-compact-section cr-overview-preferences"><h2 className="cr-section-title">Новые пожелания</h2>{newWishes.slice(0, 3).map(wish => <div className="cr-compact-row" key={wish.id} role="button" tabIndex={0} onClick={onOpenWishes} onKeyDown={event => { if (event.key === "Enter") onOpenWishes(); }}><div className="cr-avatar">{wish.employeeName[0]}</div><div className="cr-compact-copy"><strong>{wish.employeeName}</strong><span>{wish.text}</span></div></div>)}{!newWishes.length && <p style={{ color: "var(--cr-muted)", fontSize: 12 }}>Новых пожеланий нет.</p>}<button className="cr-ghost cr-compact-link" onClick={onOpenWishes}>Открыть очередь <ArrowRight size={13} /></button></section>
        </div>
      </div>
    </TopLevelPageLayout>
  );
