...
export function SchedulesPage({ notification, schedules, onCreate, onOpen, onEdit, onPrint, onDuplicate, onArchive, onRemove }: Props) {
  ...
  return (
    <PageLayout width="list" title="Расписания" description={`${schedules.length} расписаний · данные сохранены локально`} notification={notification} action={<button className="cr-primary" onClick={onCreate}><Plus size={15} aria-hidden="true" /> Создать расписание</button>}>
