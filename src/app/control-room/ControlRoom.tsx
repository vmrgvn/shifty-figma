...
export function ControlRoom({ dark, language, theme, onThemeChange, onLogout }: Props) {
  ...
  const content = selectedSchedule && workspaceMatch ? <ScheduleWorkspacePage notification={notificationMenu} schedule={selectedSchedule} openIssuesInitially={new URLSearchParams(location.search).has("issues")} onBack={() => navigate("/app/schedules")} onEdit={() => void editSchedule(selectedSchedule.id)} onPrint={() => navigate(`/print/${selectedSchedule.id}`)} onPublish={() => void publish(selectedSchedule.id)} />
    : section === "overview" ? <OverviewPage language={language} notification={notificationMenu} schedules={schedules} wishes={wishes} onCreate={createSchedule} onOpenSchedule={(id, issues) => navigate(`/app/schedules/${id}${issues ? "?issues=1" : ""}`)} onOpenWishes={() => navigate("/app/wishes")} />
    : section === "schedules" ? <SchedulesPage language={language} notification={notificationMenu} schedules={schedules} onCreate={createSchedule} onOpen={id => navigate(`/app/schedules/${id}`)} onEdit={id => void editSchedule(id)} onPrint={id => navigate(`/print/${id}`)} onDuplicate={schedule => void duplicateSchedule(schedule)} onArchive={schedule => void archiveSchedule(schedule)} onRemove={schedule => void removeSchedule(schedule)} />
    : section === "wishes" ? <WishesPage language={language} notification={notificationMenu} wishes={wishes} onCreate={createSchedule} onSave={wish => void saveWish(wish)} onOpenSchedule={id => navigate(`/app/schedules/${id}`)} />
    : <SettingsPage language={language} notification={notificationMenu} onCreate={createSchedule} theme={theme} preferences={preferences} onTheme={onThemeChange} onPreferences={updatePreferences} onLogout={onLogout} />;
